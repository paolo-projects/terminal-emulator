...
export type ValueArgumentTypes =
    | ValueArgument<string>
    | ValueArgument<number>
    | ValueArgument<boolean>;
...
export default class Tokenizer {
    ...
    parseCommandLine(cmdLine: String): Command {
        ...
        for (let i = 1; i < tokens.length; i++) {
            if (tokens[i].startsWith('--')) {
                ...
                if (previousArg.length > 0) {
                    args.push(
                        this.getValueArgFromString(previousArg, tokens[i])
                    );
                    previousArg = '';
                } else {
                    args.push(
                        this.getPositionalArgFromString(
    ...
    private getPositionalArgFromString(index: number, value: string) {
        if (!isNaN(Number(value))) {
            return new PositionalArgument<number>(index, Number(value));
        } else if (value.toLowerCase() === 'true') {
            return new PositionalArgument<boolean>(index, true);
        } else if (value.toLowerCase() === 'false') {
            return new PositionalArgument<boolean>(index, false);
        }
        return new PositionalArgument<string>(index, value);
    }
}
